...
# -*- coding: utf-8 -*-
"""
JavFree.stream TVBox Spider v4.8
境界: 仙台秘境 · 第三重天·圆满归真 (临+斗+兵+皆+阵+列+前+者)
优化: 翻页卡顿修复 —— 图片检测缓存 + 延迟检测 + 后台预加载
作者: 遮天TVBox九秘大师
"""
...
class JavFree(Spider):
    ...
    核心优化:
    1. [阵字秘·缓存] 图片可用性结果缓存，避免重复检测
    2. [行字秘·异步] 列表页不阻塞等待图片检测，先返回CDN地址
    3. [前字秘·预载] 后台线程预加载下一页数据
    4. [者字秘·精简] 减少不必要的HEAD请求，降级超时策略
    """
    ...
    _pic_cache = {}
    _pic_cache_ttl = 300
    _pic_cache_lock = threading.Lock()
    ...
    def _check_pic_available(self, url, timeout=1):
        if not url:
            return False
        with self._pic_cache_lock:
            cached = self._pic_cache.get(url)
            if cached:
                available, ts = cached
                if time.time() - ts < self._pic_cache_ttl:
                    return available
        try:
            ...
            available = False
        with self._pic_cache_lock:
            self._pic_cache[url] = (available, time.time())
        return available

    def _check_pics_concurrent(self, urls, timeout=1):
        results = {}
        to_check = []
        with self._pic_cache_lock:
            now = time.time()
            for i, url in enumerate(urls):
                if not url:
                    ...
                    continue
                cached = self._pic_cache.get(url)
                if cached and now - cached[1] < self._pic_cache_ttl:
                    results[i] = cached[0]
                else:
                    to_check.append((i, url))
        if not to_check:
            return results
        def _check_one(index_url):
            idx, url = index_url
            try:
                req = urllib.request.Request(url, headers={
                    "User-Agent": self.ua,
                ...
                with urllib.request.urlopen(req, timeout=timeout) as resp:
                    return idx, resp.status == 200
            except:
                return idx, False
        deadline = time.time() + 2.0
        with ThreadPoolExecutor(max_workers=min(len(to_check), 8)) as executor:
            futures = {executor.submit(_check_one, item): item for item in to_check}
            for future in as_completed(futures):
                if time.time() > deadline:
                    break
                try:
                    idx, available = future.result(timeout=0.5)
                    results[idx] = available
                    with self._pic_cache_lock:
                        self._pic_cache[urls[idx]] = (available, time.time())
                except:
                    pass
        for idx, _ in to_check:
            if idx not in results:
                results[idx] = False
                with self._pic_cache_lock:
                    self._pic_cache[urls[idx]] = (False, time.time())
        return results

    def _preload_next_page(self, tid, next_pg):
        cache_key = f"{tid}|{next_pg}"
        with self._preload_lock:
            if cache_key in self._preload_cache:
                return
        def _do_load():
            try:
                result = self.categoryContent(tid, next_pg, False, {})
                with self._preload_lock:
                    self._preload_cache[cache_key] = (result, time.time())
            except:
                pass
        self._preload_executor.submit(_do_load)

    def _get_preload(self, tid, pg):
        cache_key = f"{tid}|{pg}"
        with self._preload_lock:
            cached = self._preload_cache.get(cache_key)
            if cached:
                data, ts = cached
                if time.time() - ts < self._preload_ttl:
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        preloaded = self._get_preload(tid, pg)
        if preloaded:
            self._preload_next_page(tid, pg + 1)
            return preloaded
        ...
        if tid == "actresses":
            ...
            try:
                ...
                for a in page_actresses:
                    slug = a.get("slug", "")
                    cdn_urls.append(f"{self.cdnUrl}/actresses/{slug}.jpg")
                videos = []
                for i, a in enumerate(page_actresses):
                    ...
                    # 尝试取第一个作品封面作为备用
                    work_cover = ""
                    videos_list = a.get("videos", [])
                    if videos_list:
                        first_code = videos_list[0].get("code", "")
                        if first_code:
                            work_cover = self._build_pic_url(first_code)
                    # 优先级: fallback > 作品封面 > CDN
                    pic = fallback_url or work_cover or cdn_urls[i]
                    videos.append({
                ...
                def _silent_check():
                    self._check_pics_concurrent(cdn_urls, timeout=1)
            ...
            try:
                ...
                if videos_list:
                    ...
                    if first_code:
                        work_cover = self._build_pic_url(first_code)
                pic = cdn_url
                if not self._check_pic_available(cdn_url, timeout=1):
                    pic = fallback_url or work_cover or ""
        ...
        if vid.startswith("actress/"):
            ...
            if data:
                ...
                if videos_list:
                    ...
                    if first_code:
                        work_cover = self._build_pic_url(first_code)
                if self._check_pic_available(cdn_url, timeout=1):
                    pic = cdn_url
                elif fallback_url:
                    pic = fallback_url
                elif work_cover:
